...
from utils.draw_amass_tool import draw_pic_single
...
def test_func(gt3d):
    ...
    gt_future = gt3d[:, config.t_his:, dataset.joint_used].clone()
    ...
    #
    pred32 = gt3d[:, :, dataset.joint_used].clone()
    pred32[:, config.t_his:config.t_his + config.t_pred] = sampled_future[:, :config.t_pred]
    # pred32 = pred32.reshape([-1, config.t_his + config.t_pred, 24, 3])
    gt3d_t = gt3d[:, :, dataset.joint_used].contiguous()
