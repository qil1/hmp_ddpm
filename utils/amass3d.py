...
class Datasets(Dataset):
    ...
    def __init__(self, opt, split=0):
        ...
        skel = np.load('./utils/smpl_skeleton.npz')
        ...
        for ds in amass_splits[split]:
            ...
            for sub in os.listdir(self.path_to_data + '/' + ds):
                ...
                for act in os.listdir(self.path_to_data + '/' + ds + '/' + sub):
                    ...
                    if split == 2:
                        valid_frames = np.arange(0, fn - 35 + 1, opt.skip_rate)
                    else:
                        valid_frames = np.arange(0, fn - 35 + 1, opt.skip_rate)
    ...
    def __getitem__(self, item):
        ...
        le = len(self.p3d[key])
        if start_frame + self.in_n + self.out_n > le:
            fs = list(np.arange(start_frame, le))
            fs.extend([start_frame] * (start_frame + self.in_n + self.out_n - le))
            fs = np.array(fs)
        else:
            fs = np.arange(start_frame, start_frame + self.in_n + self.out_n)

        return self.p3d[key][fs] * 1000  # , key
